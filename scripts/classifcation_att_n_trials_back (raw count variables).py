...
df.loc[:,'attention' ] = df.loc[:,'attention' ].values - 1
...
for participant, df_sub in df.groupby('participant'):
    ...
    def text(x):
        ...
        confidence = {0:'low confidence',
                      1:'high confidence'}
        attention = {0:'low attention',
                     1:'high attention'}
        return f"{aware[x['awareness']]} and {confidence[x['confidence']]} --> {attention[x['attention']]}"
...
res_melt.to_csv(os.path.join(saving_dir,'att.csv'),index=False)
