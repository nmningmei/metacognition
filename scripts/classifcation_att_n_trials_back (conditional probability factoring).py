# -*- coding: utf-8 -*-
"""
Created on Thu May 23 13:56:54 2019
...
#        print(np.sum(idx_pick))
        ...
        if np.sum(idx_pick) > 0:
            for attention in [0.,1.]:
                j += 1
                idx_pick = df_pick['attention'] == attention
...
results_to_save.to_csv(os.path.join(saving_dir,'att.csv'),index=False)
...
df_plot['attention'] = df_plot['attention'].map({1.:'high att',0.:'low att'})
df_plot['level'] = df_plot['correctness'] + ', '+df_plot['awareness'] +', '+ df_plot['confidence']

#temp = []
#for (target,subject),df_sub in df_plot.groupby(['attention','sub']):
#    df_sub['prob'] = df_sub['count'] / df_sub['count'].sum()
#    temp.append(df_sub)
#df_plot = pd.concat(temp)
df_plot.to_csv(os.path.join(saving_dir,'att_for_plot.csv'))
...
g.savefig(os.path.join(figure_dir,'att.png'),
          dpi = 400,
          bbox_inches = 'tight')
...
#        print(df_sub_sub.shape)
    ...
    aovrm = AnovaRM(df_sub, 
                    'prob', 
                    'sub', within=['awareness','confidence','correctness'])
