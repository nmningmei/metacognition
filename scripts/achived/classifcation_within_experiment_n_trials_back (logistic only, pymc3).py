...
import numpy as np
from utils import bayesian_logistic,resample_ttest,MCPConverter
...
for n_back in range(11): # loop through the number of trials looking back
    for participant,df_sub in df.groupby('participant'):# for each subject
        ...
        results,trace          = bayesian_logistic(df_sub,
                                          feature_names,
                                          target_name,
                                          results,
                                          participant,
...
c.to_csv('../results/Pos_logistic.csv',index=False) # save as a csv
c = pd.read_csv('../results/Pos_logistic.csv')
...
for model,d_ in d.groupby('model'): # for each model
    ...
    temp.append(d_)
d    = pd.concat(temp)
d.to_csv('../results/Pos_ttest_logistic.csv',index = False)
...
c.to_csv('../results/ATT_logstic.csv',index = False)
c = pd.read_csv('../results/ATT_logstic.csv')
ttest = dict(model   =[],
             window  =[],
             ps_mean =[],
             ps_std  =[])
...
for model,d_ in d.groupby('model'):
    ...
    temp.append(d_)
d            = pd.concat(temp)
d.to_csv('../results/ATT_ttest_logstic.csv',index = False)
