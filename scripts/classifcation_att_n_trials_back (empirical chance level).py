...
c.to_csv(os.path.join(saving_dir,'att_6_features (empirical chance)_{}.csv'.format(participant)),index=False) # save as a csv
################################################################################
# use attention, awareness, and confidence as features
np.random.seed(12345)
c = []
...
c = pd.concat(c) # concate
#c = c.groupby(groupby).mean().reset_index()
c.to_csv(os.path.join(saving_dir,'att_3_1_features (empirical chance)_{}.csv'.format(participant)),index=False) # save as a csv
###############################################################################
...
c = pd.concat(c) # concate
#c = c.groupby(groupby).mean().reset_index()
c.to_csv(os.path.join(saving_dir,'att_RT_features (empirical chance)_{}.csv'.format(participant)),index=False) # save as a csv
