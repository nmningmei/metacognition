...
#subs = ['ack', 'cc', 'ck', 'cpj', 'em', 'es', 'fd', 'jmac', 'lidia', 'ls','mimi', 'pr', 'pss', 'sva', 'tj']
for participant in subs:
    with open(os.path.join(batch_dir,'correlate predicted awareness with correctness e2 ({}).py'.format(participant)),'wb') as new_file:
        with open('correlate predicted awareness with correctness e2.py','rb') as old_file:
            for line in old_file:
                new_file.write(line.replace("participant     = 'ack'","participant = '{}'".format(participant)))
...
# This is a script to send correlate predicted awareness with correctness e2 ({}).py as a batch job.
# it works on dataset {}
...
python "correlate predicted awareness with correctness e2 ({}).py"
"""
for participant in subs:
    with open(os.path.join(batch_dir,'model_comparison_e2_{}'.format(participant)),'w') as f:
        f.write(content.format(participant,participant,participant,participant,participant,participant,participant))
...
'''
with open(os.path.join(batch_dir,'qsub_jobs_e2'),'w') as f:
    f.write(content)

with open(os.path.join(batch_dir,'qsub_jobs_e2'),'a') as f:
    for participant in subs:
        f.write('qsub model_comparison_e2_{}\n'.format(participant))
