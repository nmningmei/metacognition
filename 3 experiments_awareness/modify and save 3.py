...
#subs = ['ab', 'eb', 'er', 'hgh', 'kb', 'kj', 'mp', 'rb', 'vs', 'wp']
for participant in subs:
    with open(os.path.join(batch_dir,'correlate predicted awareness with correctness e3 ({}).py'.format(participant)),'wb') as new_file:
        with open('correlate predicted awareness with correctness e3.py','rb') as old_file:
            for line in old_file:
                new_file.write(line.replace("participant     = 'ab'","participant = '{}'".format(participant)))
...
# This is a script to send correlate predicted awareness with correctness e3 ({}).py as a batch job.
# it works on dataset {}
...
python "correlate predicted awareness with correctness e3 ({}).py"
"""
for participant in subs:
    with open(os.path.join(batch_dir,'model_comparison_e3_{}'.format(participant)),'w') as f:
        f.write(content.format(participant,participant,participant,participant,participant,participant,participant))
...
'''
with open(os.path.join(batch_dir,'qsub_jobs_e3'),'w') as f:
    f.write(content)

with open(os.path.join(batch_dir,'qsub_jobs_e3'),'a') as f:
    for participant in subs:
        f.write('qsub model_comparison_e3_{}\n'.format(participant))
